...
public class Validate {
	...
	public static boolean checkIDExists(String id, ArrayList<ManageEmployee> arrayList) {
		boolean flag = false;
		
		if(arrayList.isEmpty() != true) {
			for(int count = 0; count < arrayList.size(); count++) {
				String option = arrayList.get(count).employeeID;
	...
	public static boolean isValidID(String id) {
		...
		if(id.length() != 7) {
			System.out.println("Error: Employee ID should have 7 character starting with ace followed by four digit.\n");
			idFlag = false;
		}
			...
			System.out.println("Error: Employee id shouldn't have three or more consecutive character.\nThe prefix should be ace\n");
			idFlag = false;
		}
		
		else if(isSameSequence(id.substring(0, 3))) {
			System.out.println("Error: Employee id shouldn't have three or more same consecutive character.\nThe prefix should be ace\n");
			idFlag = false;
		}
		
		if(matcher.matches() == true) {
			// if(checkIDExists(id, details)) {
			...
			System.out.println("Error: Employee ID must start with three alphabets which is (ace) followed by four numeric example ace1234.\nAnd no same consecutive are allowed for alphabets.\nNo special characters are allowed.\n");
			idFlag = false;
		}
	...
	public static boolean isValidEmail(String email) {
		...
		// ^[a-zA-Z0-9]+(?:\\." + "[a-zA-Z0-9_+&*-]+)*@" + "(?:[a-zA-Z-]+\\.)+[a-z" + "A-Z]{2,7}$
		
		String expression = "^([a-zA-Z][a-zA-Z0-9]*)+(?:\\." + "[a-zA-Z0-9]+)*@" 
								+ "(?:[a-zA-Z-]+\\.)+[a-z" + "A-Z]{2,7}$";
	...
	public static Date isValidDateOfBirth(String dob) {
		...
		if(dob.matches(pattern)) {
			try {
				...
				System.out.println("Error: "+dob+" is in invalid date format. Please enter date in the format of dd/mm/yyyy.\n");
		        dobFlag = false;
			} 
			
			if(dobDate.after(todaysDate)) {
				System.out.println("Error: "+dob+" is a future date and date of birth shouldn't be a future date.\n");
				dobFlag = false;
			}
			
			else if(dobDate.equals(todaysDate)) {
				System.out.println("Error: "+dob+" is the current date and date of birth shouldn't be a todays date.\n");
				dobFlag = false;
			}
			...
			if(yearInDifference < 18L) {
				dobFlag = false;
				System.out.println("Error: Age of the employee shouldn't be less than 18! Enter a valid date of birth.\n");
			}
			
			else if(yearInDifference > 60L) {
				dobFlag = false;
				System.out.println("Error: Age of the employee shouldn't be more than 60! Enter a valid date of birth.\n");
			}
		}
	...
	public static Date isValidDateOfJoining(String doj, Date dobDate) {
		...
		if(doj.matches(pattern)) {
			try {
				...
				System.out.println("Error: "+doj+" is in invalid date format. Please enter date in the format of dd/mm/yyyy.\n");
		        dojFlag = false;
			} 
			
			if(dojDate.after(todaysDate)) {
				System.out.println("Error: "+doj+" is a future date and date of birth shouldn't be a future date.\n");
				dojFlag = false;
			}
			
			else if(dojDate.equals(todaysDate)) {
				System.out.println("Error: "+doj+" is the current date and date of birth shouldn't be a todays date.\n");
				dojFlag = false;
			}
			
			else if(dojDate.equals(dobDate)) {
				 System.out.println("Error: Date of joining: " + format.format(dobDate) + " and Date of birth: " +
				 doj +" shouldn't be same\n"); 
				 dojFlag = false; 
			}
